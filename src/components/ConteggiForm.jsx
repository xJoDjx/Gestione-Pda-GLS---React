...
// ─── TABELLA VOCI GENERICA ────────────────────────────────────────────────────
// Riutilizzata per spedizioni, consegne doppie, palmari, compensazioni distribuzione
const TabellaVoci = ({ voci, onChange, colImporto="Importo €", accentColor="#3b82f6", negativo=false, labelBtn="Aggiungi voce" }) => {
  const is = { width:"100%",padding:"3px 6px",borderRadius:5,border:"1px solid #e2e8f0",fontSize:11,boxSizing:"border-box" };
  const aggiorna = (i, patch) => { const a=[...voci]; a[i]={...a[i],...patch}; onChange(a); };
  const rimuovi  = (i) => onChange(voci.filter((_,j)=>j!==i));
  const aggiungi = () => onChange([...voci,{descrizione:"",importo:0,note:""}]);
    ...
    <div style={{ display:"flex",flexDirection:"column",gap:6 }}>
          ...
          <table style={{ width:"100%",borderCollapse:"collapse" }}>
            ...
            <thead>
              <tr style={{ background:"#f8fafc" }}>
                {["Descrizione",colImporto,"Note",""].map(h=>(
              ...
              {voci.map((voce,i)=>(
                <tr key={i} style={{ background:i%2===0?"#fff":"#fafafa" }}>
                  <td style={{ padding:"4px 5px",borderBottom:"1px solid #f1f5f9" }}>
                    <input value={voce.descrizione||""} onChange={e=>aggiorna(i,{descrizione:e.target.value})}
                      placeholder="Descrizione" style={is} />
                  </td>
                  <td style={{ padding:"4px 5px",borderBottom:"1px solid #f1f5f9" }}>
                    <input type="number" value={voce.importo||0} step="0.01"
                      onChange={e=>{ let v=parseFloat(e.target.value)||0; if(negativo&&v>0)v=-v; aggiorna(i,{importo:v}); }}
                      style={{...is,fontFamily:"'DM Mono',monospace",color:negativo?"#dc2626":"inherit",background:negativo?"#fff1f2":"#fff",border:negativo?"1px solid #fca5a5":"1px solid #e2e8f0"}} />
                  </td>
                  <td style={{ padding:"4px 5px",borderBottom:"1px solid #f1f5f9" }}>
                    <input value={voce.note||""} onChange={e=>aggiorna(i,{note:e.target.value})}
                      placeholder="note..." style={{...is,color:"#64748b"}} />
                  </td>
      ...
      <div style={{ display:"flex",justifyContent:"space-between",alignItems:"center",gap:8 }}>
        <button onClick={aggiungi}
          ...
          <Icon name="plus" size={13}/> {labelBtn}
        </button>
        {voci.length>0 && (
          <div style={{ fontSize:11,color:accentColor,fontFamily:"'DM Mono',monospace",fontWeight:700,padding:"4px 10px",background:accentColor+"18",borderRadius:6 }}>
            Totale: {euro(voci.reduce((s,v)=>s+(v.importo||0),0))}
          </div>
        )}
      </div>
    </div>
...
export const ConteggiForm = ({ form, setForm, padroncino, mese, anno, giorni, onSave, onSaveTemplate, embedded, addebiti_standard=[], ricaricheMese={}, mezziFlotta=[] }) => {
  ...
  const calcolaImportoRicarica = (targa) => {
    ...
    // Maggiorazione: per-mezzo ha priorità, poi quella del mese, poi default 20%
    const tuttiMezzi  = mezziFlotta.length > 0 ? mezziFlotta : (padroncino?.mezzi || []);
    const mz          = tuttiMezzi.find(m => (m.targa||"").toUpperCase() === (targa||"").toUpperCase());
    const maggiPct    = mz?.maggiorazione_ricarica_pct != null
      ? mz.maggiorazione_ricarica_pct
      : (ricaricheMese.maggiorazione_pct ?? 20);
  ...
  // Auto-inizializza voci_palmari con la riga calcolata se vuota e il calcolo è disponibile
  const initPalmariDone = useRef(false);
  useEffect(() => {
    if (initPalmariDone.current) return;
    if ((form.voci_palmari||[]).length > 0) { initPalmariDone.current = true; return; }
    if (!form.addebiti_palmari && !(form.n_palmari > 0)) return;
    initPalmariDone.current = true;
    const importoCalc = parseFloat(((form.n_palmari||0) * PALMARE_TARIFFA_GG * giorni).toFixed(2));
    if (importoCalc <= 0) return;
    const descCalc = `${form.n_palmari||0} palmar${(form.n_palmari||0)===1?"e":"i"} × €${PALMARE_TARIFFA_GG}/gg × ${giorni} gg`;
    set("voci_palmari", [{ descrizione: descCalc, importo: importoCalc, note: "" }]);
  }, [form.addebiti_palmari, form.n_palmari]);

  // Auto-calcola totale spedizioni dalle voci
  const prevVociSped = useRef(JSON.stringify(form.voci_spedizioni));
  useEffect(() => {
    const curr = JSON.stringify(form.voci_spedizioni);
    if (curr === prevVociSped.current) return;
    prevVociSped.current = curr;
    const tot = parseFloat((form.voci_spedizioni||[]).reduce((s,v)=>s+(v.importo||0),0).toFixed(2));
    set("totale_spedizioni", tot);
  });

  // Auto-calcola totale consegne doppie dalle voci
  const prevVociCD = useRef(JSON.stringify(form.voci_consegne_doppie));
  useEffect(() => {
    const curr = JSON.stringify(form.voci_consegne_doppie);
    if (curr === prevVociCD.current) return;
    prevVociCD.current = curr;
    const tot = parseFloat((form.voci_consegne_doppie||[]).reduce((s,v)=>s+(v.importo||0),0).toFixed(2));
    set("consegne_doppie", tot>0?-tot:tot);
  });

  // Auto-calcola addebiti palmari dalle voci
  const prevVociPalm = useRef(JSON.stringify(form.voci_palmari));
  useEffect(() => {
    const curr = JSON.stringify(form.voci_palmari);
    if (curr === prevVociPalm.current) return;
    prevVociPalm.current = curr;
    const tot = parseFloat((form.voci_palmari||[]).reduce((s,v)=>s+(v.importo||0),0).toFixed(2));
    if ((form.voci_palmari||[]).length) set("addebiti_palmari", tot);
  });

  // Auto-calcola compensazioni distribuzione dalle voci
  const prevVociComp = useRef(JSON.stringify(form.voci_compensazioni_distribuzione));
  useEffect(() => {
    const curr = JSON.stringify(form.voci_compensazioni_distribuzione);
    if (curr === prevVociComp.current) return;
    prevVociComp.current = curr;
    const tot = parseFloat((form.voci_compensazioni_distribuzione||[]).reduce((s,v)=>s+(v.importo||0),0).toFixed(2));
    set("compensazioni_distribuzione", tot);
  });
      ...
      {tab==="fatturato" && (
        <div style={{ display:"grid",gridTemplateColumns:"1fr 1fr",gap:14 }}>
          <SectionCard title="Fatturato" icon="euro" accent="#3b82f6">
            <div style={{ display:"flex",flexDirection:"column",gap:14 }}>
              {/* ── Totale Spedizioni / Proforma ── */}
              <div style={{ display:"flex",flexDirection:"column",gap:6 }}>
                <label style={{ fontSize:11,fontWeight:700,color:"#374151",textTransform:"uppercase",letterSpacing:"0.06em" }}>Totale Spedizioni – Valore Proforma</label>
                <TabellaVoci
                  voci={form.voci_spedizioni||[]}
                  onChange={voci=>{
                    set("voci_spedizioni",voci);
                    set("totale_spedizioni",parseFloat(voci.reduce((s,v)=>s+(v.importo||0),0).toFixed(2)));
                  }}
                  colImporto="Importo €"
                  accentColor="#1d4ed8"
                  labelBtn="Aggiungi riga spedizione"
                />
              </div>

              {/* ── Consegne Doppie ── */}
              <div style={{ display:"flex",flexDirection:"column",gap:6,borderTop:"1px solid #f1f5f9",paddingTop:10 }}>
                <label style={{ fontSize:11,fontWeight:700,color:"#dc2626",textTransform:"uppercase",letterSpacing:"0.06em" }}>Consegne Doppie (a scalare)</label>
                <TabellaVoci
                  voci={form.voci_consegne_doppie||[]}
                  onChange={voci=>{
                    set("voci_consegne_doppie",voci);
                    const tot=parseFloat(voci.reduce((s,v)=>s+(v.importo||0),0).toFixed(2));
                    set("consegne_doppie",tot>0?-tot:tot);
                  }}
                  colImporto="Importo € (neg.)"
                  accentColor="#dc2626"
                  negativo={true}
                  labelBtn="Aggiungi riga consegna doppia"
                />
              </div>
          ...
          <SectionCard title="Palmari" icon="device" accent="#f59e0b">
            <div style={{ display:"flex",flexDirection:"column",gap:10 }}>
              ...
              {(padroncino?.palmari||[]).length > 0 && (
                <div style={{ fontSize:11,color:"#92400e",padding:"7px 10px",background:"#fffbeb",borderRadius:7,border:"1px solid #fde68a",display:"flex",alignItems:"center",gap:5 }}>
                  📱 <span>Da anagrafica: <strong>{(padroncino?.palmari||[]).filter(p=>p.stato==="ATTIVO").length} attivi</strong> · {form.n_palmari} × €{PALMARE_TARIFFA_GG}/gg × {giorni}gg = <strong style={{ fontFamily:"'DM Mono',monospace" }}>{euro(form.addebiti_palmari)}</strong></span>
                </div>
              )}
              <TabellaVoci
                voci={form.voci_palmari||[]}
                onChange={voci=>{
                  set("voci_palmari",voci);
                  set("addebiti_palmari",parseFloat(voci.reduce((s,v)=>s+(v.importo||0),0).toFixed(2)));
                }}
                colImporto="Importo €"
                accentColor="#f59e0b"
                labelBtn="Aggiungi voce palmare"
              />
              {(form.voci_palmari||[]).length > 0 && (
                <div style={{ fontSize:11,color:"#64748b",padding:"5px 8px",background:"#f8fafc",borderRadius:6,display:"flex",justifyContent:"space-between" }}>
                  <span>+IVA 22%</span>
                ...
                <div style={{ padding:"8px 12px",background:"#e0f2fe",borderRadius:8,fontSize:12,color:"#0c4a6e",display:"flex",alignItems:"center",justifyContent:"space-between" }}>
                  ...
                  <button onClick={() => {
                    ...
                    mezziElettrici.forEach(m => {
                      ...
                      if (idx >= 0) { aggiornate[idx] = { ...aggiornate[idx], importo, descrizione }; }
                      else { aggiornate.push({ targa: m.targa, descrizione, importo, iva_rate: 0.22 }); }
                    });
      ...
      {tab==="compensazioni" && (
        <div style={{ display:"grid",gridTemplateColumns:"1fr 1fr",gap:14 }}>
          <SectionCard title="Compensazioni su Distribuzione" icon="euro" accent="#0ea5e9">
            <div style={{ display:"flex",flexDirection:"column",gap:8 }}>
              <TabellaVoci
                voci={form.voci_compensazioni_distribuzione||[]}
                onChange={voci=>{
                  set("voci_compensazioni_distribuzione",voci);
                  set("compensazioni_distribuzione",parseFloat(voci.reduce((s,v)=>s+(v.importo||0),0).toFixed(2)));
                }}
                colImporto="Importo €"
                accentColor="#0ea5e9"
                labelBtn="Aggiungi voce compensazione"
              />
            </div>
          </SectionCard>
        ...
        <div style={{ display:"grid", gridTemplateColumns:"1fr 300px", gap:16, alignItems:"start" }}>
          ...
          <div style={{ display:"flex",flexDirection:"column",gap:12 }}>
            ...
            <RiepilogoSezione
              titolo="Fatturato"
              emoji="📈"
              totale={euro(form.totale_fattura)}
              colore="#1d4ed8"
              borderColor="#dbeafe"
              bgHeader="#eff6ff"
            >
              {/* Totale Spedizioni: mostra voci se presenti, altrimenti valore singolo */}
              {(form.voci_spedizioni||[]).length > 0
                ? (form.voci_spedizioni||[]).filter(v=>(v.importo||0)!==0).map((v,i)=>(
                    <RigaVoce key={i} label={v.descrizione||"Spedizione"} value={v.importo||0} nota={v.note} color="#0369a1" bg="#eff6ff" accent="#bfdbfe" indent />
                  ))
                : (form.totale_spedizioni||0)!==0 && (
                    <RigaVoce label="Totale Spedizioni / Proforma" value={form.totale_spedizioni||0} nota={form.note_spedizioni} color="#374151" />
                  )
              }
              {/* Consegne Doppie: mostra voci se presenti, altrimenti valore singolo */}
              {(form.voci_consegne_doppie||[]).length > 0
                ? (form.voci_consegne_doppie||[]).filter(v=>(v.importo||0)!==0).map((v,i)=>(
                    <RigaVoce key={i} label={v.descrizione||"Consegna Doppia"} value={v.importo||0} nota={v.note} color="#dc2626" bg="#fff1f2" accent="#fca5a5" indent />
                  ))
                : (form.consegne_doppie||0)!==0 && (
                    <RigaVoce label="Consegne Doppie" value={form.consegne_doppie||0} color="#374151" />
                  )
              }
              {[
                ...
                <RigaVoce key={l} label={l} value={v} nota={nota} color="#374151" />
              ))}
              {(form.voci_fatturato||[]).filter(v=>(v.val||0)!==0).map((v,i)=>(
                <RigaVoce key={i} label={v.label||"PDA Extra"} value={v.val||0} nota={v.note} color="#0369a1" bg="#f0f9ff" accent="#bae6fd" indent />
              ))}
              {(form.altri_fatturato||[]).filter(v=>(v.importo||0)!==0).map((v,i)=>(
                <RigaVoce key={i} label={v.descrizione||"Extra"} value={v.importo||0} nota={v.note} color="#6d28d9" bg="#faf5ff" accent="#c4b5fd" indent />
              ))}
              <div style={{ borderTop:"1px solid #dbeafe",marginTop:6,paddingTop:6,display:"flex",flexDirection:"column",gap:2 }}>
                <RigaVoce label="Imponibile" value={form.totale_imponibile} color="#374151" bold />
                <RigaVoce label="IVA 22%" value={form.iva} color="#64748b" />
                {(form.acconto_fattura||0)>0 && (
                  <RigaVoce label="– Acconto su Fattura" value={-(form.acconto_fattura||0)} color="#dc2626" prefix="" />
                )}
              </div>
            </RiepilogoSezione>
            ...
            <RiepilogoSezione
              titolo="Addebiti"
              emoji="📤"
              totale={`–${euro(form.totale_addebiti)}`}
              colore="#dc2626"
              borderColor="#fecaca"
              bgHeader="#fef2f2"
            >
                ...
                <>
                  ...
                  {(form.voci_palmari||[]).length > 0
                    ? (form.voci_palmari||[]).filter(v=>(v.importo||0)!==0).map((v,i)=>(
                        <RigaVoce
                          key={i}
                          label={`📱 ${v.descrizione||"Palmare"}`}
                          value={v.importo||0}
                          nota={v.note}
                          color="#92400e" bg="#fffbeb" accent="#fde68a" indent bold
                        />
                      ))
                    : (
                        <RigaVoce
                          label={`📱 ${form.n_palmari||0}× Palmari × ${giorni} gg (+IVA 22%)`}
                          value={palmariIvato} color="#92400e" bg="#fffbeb" accent="#fde68a" indent bold
                        />
                      )
                  }
                </>
              ...
              {((form.altri_addebiti||[]).filter(a=>(a.importo||0)>0).length>0||addebitiDet.bollo>0) && (
                <>
                  ...
                  {addebitiDet.bollo>0 && (
                    <RigaVoce label="📮 Bollo virtuale" value={addebitiDet.bollo} color="#92400e" bg="#fffbeb" accent="#fde68a" indent />
                  )}
                </>
              )}
            </RiepilogoSezione>
            ...
            {((form.compensazioni_distribuzione||0)!==0||(form.fatture_fine_mese||[]).some(f=>(f.importo||0)>0)||cassaNorm.some(cc=>(cc.importo||0)>0)) && (() => {
              ...
              return (
                <RiepilogoSezione
                  titolo="Compensazioni & Deduzioni"
                  emoji="⚖️"
                  totale={`–${euro(totComp)}`}
                  colore="#0369a1"
                  borderColor="#bae6fd"
                ...
                >
                  {(form.compensazioni_distribuzione||0)!==0 && (
                    (form.voci_compensazioni_distribuzione||[]).length > 0
                      ? (form.voci_compensazioni_distribuzione||[]).filter(v=>(v.importo||0)!==0).map((v,i)=>(
                          <RigaVoce key={i} label={v.descrizione||"Compensazione"} value={v.importo||0} nota={v.note} color="#0369a1" bg="#f0f9ff" accent="#7dd3fc" indent />
                        ))
                      : <RigaVoce label="Compensaz. su Distribuzione" value={form.compensazioni_distribuzione} color="#0369a1" bg="#f0f9ff" accent="#7dd3fc" indent />
                  )}
            ...
            {(() => {
              const totComp=(form.compensazioni_distribuzione||0)
                +((form.fatture_fine_mese||[]).reduce((s,f)=>s+(f.importo||0),0))
                +(cassaNorm.reduce((s,cc)=>s+(cc.importo||0),0));
              const positivo = form.totale_da_bonificare >= 0;
              return (
                <div style={{ borderRadius:14,border:`2px solid ${positivo?"#86efac":"#fca5a5"}`,overflow:"hidden" }}>
                  {/* Dettaglio calcolo */}
                  <div style={{ background:"#f8fafc",padding:"12px 18px",borderBottom:"1px solid #e2e8f0" }}>
                    <div style={{ fontSize:10,fontWeight:700,color:"#94a3b8",textTransform:"uppercase",letterSpacing:"0.08em",marginBottom:8 }}>🏁 Calcolo Finale</div>
                  ...
                  {/* Totale finale */}
                  <div style={{ padding:"20px",textAlign:"center",background:positivo?"#dcfce7":"#fee2e2" }}>
                    ...
                    <div style={{ fontSize:42,fontWeight:800,fontFamily:"'DM Mono',monospace",color:positivo?"#166534":"#dc2626",letterSpacing:"-0.03em" }}>
          ...
          {/* ── Colonna destra: Checklist + Operazioni + Note ── */}
          <div style={{ display:"flex",flexDirection:"column",gap:10,position:"sticky",top:12 }}>

            {/* ── CHECKLIST ── */}
            <div style={{ background:"#fff",borderRadius:12,border:"1px solid #e2e8f0",overflow:"hidden" }}>
              <div style={{ background:"#f0fdf4",padding:"9px 14px",borderBottom:"1px solid #bbf7d0",display:"flex",alignItems:"center",gap:6 }}>
                <span style={{ fontSize:13 }}>✅</span>
                <span style={{ fontSize:11,fontWeight:800,color:"#166534",textTransform:"uppercase",letterSpacing:"0.06em" }}>Checklist</span>
                {/* Badge progresso */}
                {(() => {
                  const voci = ["distrib_inviata","fatt_tu_creata","fattura_ricevuta","pdf_addeb"];
                  const done = voci.filter(k=>form[k]).length;
                  return (
                    <span style={{ marginLeft:"auto",fontSize:10,fontWeight:700,color:done===voci.length?"#166534":"#64748b",
                      background:done===voci.length?"#dcfce7":"#f1f5f9",
                      padding:"2px 8px",borderRadius:20,border:"1px solid",
                      borderColor:done===voci.length?"#bbf7d0":"#e2e8f0" }}>
                      {done}/{voci.length}
                    </span>
                  );
                })()}
                ...
                {[
                  ...
                  <div key={key} onClick={() => set(key,!form[key])}
                    style={{ display:"flex",alignItems:"center",gap:10,padding:"8px 10px",borderRadius:8,
                      background:form[key]?"#f0fdf4":"#f8fafc",
                      border:`1px solid ${form[key]?"#86efac":"#e2e8f0"}`,cursor:"pointer",transition:"all 0.15s" }}>
                    <div style={{ width:18,height:18,borderRadius:5,flexShrink:0,
                      background:form[key]?"#10b981":"#fff",
                      border:`2px solid ${form[key]?"#10b981":"#d1d5db"}`,
                    ...
                    <span style={{ fontSize:12,fontWeight:600,color:form[key]?"#166534":"#374151",lineHeight:1.3 }}>{label}</span>
                  </div>
                ))}
              </div>
            </div>
            ...
            {/* ── OPERAZIONI ── */}
            <div style={{ background:"#fff",borderRadius:12,border:"1px solid #e2e8f0",overflow:"hidden" }}>
              <div style={{ background:"#f8fafc",padding:"9px 14px",borderBottom:"1px solid #e2e8f0",display:"flex",justifyContent:"space-between",alignItems:"center" }}>
                <div style={{ display:"flex",alignItems:"center",gap:6 }}>
                  <span style={{ fontSize:13 }}>📋</span>
                  <span style={{ fontSize:11,fontWeight:800,color:"#374151",textTransform:"uppercase",letterSpacing:"0.06em" }}>Operazioni</span>
                </div>
                <button
              ...
              <div style={{ padding:"10px 12px",display:"flex",flexDirection:"column",gap:6,minHeight:50 }}>
                {(form.ops_mese||[]).length===0 && (
                  <div style={{ textAlign:"center",padding:"12px 0",color:"#94a3b8",fontSize:11 }}>
                    Nessuna operazione. Clicca + Aggiungi.
                  </div>
                )}
            ...
            <div style={{ background:"#fff",borderRadius:12,border:"1px solid #fde68a",overflow:"hidden" }}>
              <div style={{ background:"#fffbeb",padding:"9px 14px",borderBottom:"1px solid #fde68a",display:"flex",alignItems:"center",gap:6 }}>
                <span style={{ fontSize:13 }}>📝</span>
                <span style={{ fontSize:11,fontWeight:800,color:"#92400e",textTransform:"uppercase",letterSpacing:"0.06em" }}>Note rapide</span>
              </div>
              <textarea value={form.note_varie||""} onChange={e=>set("note_varie",e.target.value)}
                ...
                style={{ width:"100%",minHeight:100,padding:"10px 12px",border:"none",fontSize:11,resize:"vertical",boxSizing:"border-box",fontFamily:"inherit",outline:"none",color:"#374151",lineHeight:1.5 }}/>
            </div>
